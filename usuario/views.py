...
class StreetLeaderListView(ListView):
    model = StreetLeader
    template_name = 'street.leader.list.html'
    ...
    def get_queryset(self):
        ...
        if Nacional.objects.filter(perfil=self.request.user.perfil):
            nacional = Nacional.objects.get(perfil=self.request.user.perfil)
            queryset = StreetLeader.objects.filter(jefe_clap__clap__parroquia__municipio__estado__pais=nacional.pais)
            return queryset

        ## usuario estadal puede ver al nivel comunal
        if Estadal.objects.filter(perfil=self.request.user.perfil):
            estadal = Estadal.objects.get(perfil=self.request.user.perfil)
            queryset = StreetLeader.objects.filter(jefe_clap__clap__parroquia__municipio__estado=estadal.estado)
            return queryset

        ## usuario municipal puede ver al comunal
        if Municipal.objects.filter(perfil=self.request.user.perfil):
            municipal = Municipal.objects.get(perfil=self.request.user.perfil)
            queryset = StreetLeader.objects.filter(jefe_clap__clap__parroquia__municipio=municipal.municipio)
            return queryset

        ## usuario parroquial puede ver al comunal
        if Parroquial.objects.filter(perfil=self.request.user.perfil):
            parroquial = Parroquial.objects.get(perfil=self.request.user.perfil)
            queryset = StreetLeader.objects.filter(jefe_clap__clap__parroquia=parroquial.parroquia)
            return queryset
        ...
        if JefeClap.objects.filter(perfil=self.request.user.perfil):
            ...
            queryset = StreetLeader.objects.filter(jefe_clap__clap=jefe_clap.clap)
            return queryset

class StreetLeaderCreateView(CreateView):
    model = User
    form_class = PerfilForm
    template_name = 'street.leader.create.html'
    ...
    def form_valid(self, form):
        ...
        enviado = enviar_correo(self.object.email, 'usuario.bienvenida.mail', EMAIL_SUBJECT_REGISTRO, {'nivel':'Clap','nombre':self.request.user.first_name,
            'apellido':self.request.user.last_name, 'correo':self.request.user.email, 'username':self.object.username, 'clave':form.cleaned_data['password'],
            'admin':admin, 'admin_email':admin_email, 'emailapp':settings.EMAIL_FROM
        })
...
class StreetLeaderUpdateView(UpdateView):
    ...
    template_name = "street.leader.update.html"
    success_url = reverse_lazy('inicio')
    ...
    def get_initial(self):
        ...
        stree_leader = StreetLeader.objects.get(perfil=perfil)
        initial_data['clap'] = stree_leader.jefe_clap.clap
        return initial_data
...
class GrupoFamiliarList(ListView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if self.request.user.perfil.nivel == 1 or self.request.user.perfil.nivel == 2 or self.request.user.perfil.nivel == 3 or self.request.user.perfil.nivel == 4 or self.request.user.perfil.nivel == 5 or self.request.user.perfil.nivel == 6:
    ...
    def get_queryset(self):
        ...
        if Nacional.objects.filter(perfil=self.request.user.perfil):
            nacional = Nacional.objects.get(perfil=self.request.user.perfil)
            queryset = GrupoFamiliar.objects.filter(street_leader__jefe_clap__clap__parroquia__municipio__estado__pais=nacional.pais, )
            return queryset

        ## usuario estadal puede ver al nivel comunal
        if Estadal.objects.filter(perfil=self.request.user.perfil):
            estadal = Estadal.objects.get(perfil=self.request.user.perfil)
            queryset = GrupoFamiliar.objects.filter(street_leader__jefe_clap__clap__parroquia__municipio__estado=estadal.estado)
            return queryset

        ## usuario municipal puede ver al comunal
        if Municipal.objects.filter(perfil=self.request.user.perfil):
            municipal = Municipal.objects.get(perfil=self.request.user.perfil)
            queryset = GrupoFamiliar.objects.filter(street_leader__jefe_clap__clap__parroquia__municipio=municipal.municipio)
            return queryset

        ## usuario parroquial puede ver al comunal
        if Parroquial.objects.filter(perfil=self.request.user.perfil):
            parroquial = Parroquial.objects.get(perfil=self.request.user.perfil)
            queryset = GrupoFamiliar.objects.filter(street_leader__jefe_clap__clap__parroquia=parroquial.parroquia)
            return queryset

        if JefeClap.objects.filter(perfil=self.request.user.perfil):
            jefe_clap = JefeClap.objects.get(perfil=self.request.user.perfil)
            queryset = GrupoFamiliar.objects.filter(street_leader__jefe_clap__clap=jefe_clap.clap)
            return queryset
        ...
        if StreetLeader.objects.filter(perfil=self.request.user.perfil):
            ...
            queryset = GrupoFamiliar.objects.filter(street_leader__jefe_clap__clap=street_leader.jefe_clap.clap)
            return queryset
...
class GrupoFamiliarCreate(CreateView):
    ...
    def form_valid(self, form):
        ...
        enviado = enviar_correo(self.object.email, 'usuario.bienvenida.mail', EMAIL_SUBJECT_REGISTRO, {'nivel':'Jefe de calle','nombre':self.request.user.first_name,
            'apellido':self.request.user.last_name, 'correo':self.request.user.email, 'username':self.object.username, 'clave':form.cleaned_data['password'],
            'admin':admin, 'admin_email':admin_email, 'emailapp':settings.EMAIL_FROM
        })
...
class GrupoFamiliarUpdate(UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if self.request.user.id == self.kwargs['pk'] and self.request.user.perfil.nivel == 7:
    ...
    def get_initial(self):
        ...
        datos_iniciales['street_leader'] = grupo_familiar.street_leader
        return datos_iniciales
